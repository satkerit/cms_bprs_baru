...
<x-admin.page-header title="Pengaduan Nasabah" subtitle="Kelola pengaduan dan keluhan nasabah">
 ...
 target="_blank"
 class="inline-flex items-center gap-2 px-4 py-2 text-[11px] font-medium text-white bg-emerald-600 rounded-xl">
...
<x-admin.card :noPadding="true">
 ...
 <button type="button" data-open-modal="deleteComplaint{{ $complaint->id }}" class="flex-1 py-2 text-[11px] font-medium text-red-600 rounded-xl">
 Hapus
 </button>
 </div>
 </div>
...
@if($complaints->count())
 ...
 :message="'Apakah Anda yakin ingin menghapus pengaduan dari ' . json_encode($complaint->name) . '? Tindakan ini tidak dapat dibatalkan.'"
 action="{{ route('admin.customer-complaints.destroy', $complaint) }}"
 />
 @endforeach
@endif
@endsection
