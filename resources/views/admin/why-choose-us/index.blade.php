...
@section('content')
 ...
 class="inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-xl font-bold shadow-emerald-600/30 hover:-translate-y-0.5">
 <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 <div
 class="bg-white rounded-xl p-6 hover:shadow-2xl hover:shadow-blue-500/10 border flex flex-col h-full group">
 <!-- Color Bar -->
 <div class="top-0 left-0 w-full h-1 {{ $item->bg_class }}"></div>

 <div class="flex justify-between items-start mb-5">
 <div
 class="w-14 h-14 rounded-xl {{ $item->bg_class }} flex items-center justify-center {{ $item->text_class }}">
 ...
 <div class="mt-8">
 <a href="{{ route('admin.why-choose-us.create') }}"
 class="inline-flex items-center px-6 py-3 bg-sky-500 text-white rounded-xl font-bold shadow-blue-500/30 hover:-translate-y-1">
 <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
 </svg>
 Tambah Item Pertama
...
@if($items->count())
 ...
 title="Hapus Item"
 message="Apakah Anda yakin ingin menghapus item \"{{ $item->title }}\"? Tindakan ini tidak dapat dibatalkan."
 action="{{ route('admin.why-choose-us.destroy', $item) }}"
 />
 @endforeach
@endif
@endsection
