...
@section('content')
<x-admin.page-header title="Tambah Lelang Agunan" subtitle="Buat informasi lelang agunan baru untuk masyarakat">
    <x-slot:actions>
...
<form method="POST" action="{{ route('admin.auctions.store') }}" enctype="multipart/form-data" id="auction-form">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="address" id="address" rows="2"
                                  ...
                                  required placeholder="Alamat lengkap lokasi agunan...">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                                  ...
                                  placeholder="Jelaskan kondisi agunan, batas wilayah, fasilitas sekitar, atau informasi penting lainnya...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="auction_type" id="auction_type" class="w-full px-4 py-2 border dark:border-slate-700 border-zinc-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-colors text-sm @error('auction_type') border-red-500 @enderror" required>
                                ...
                                @foreach(\App\Enums\AuctionType::cases() as $aType)
                                    <option value="{{ $aType->value }}" {{ old('auction_type', 'eksekusi_hak_tanggungan') === $aType->value ? 'selected' : '' }}>
                                        {{ $aType->label() }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="auction_location" id="auction_location" value="{{ old('auction_location', 'KPKNL Pangkalpinang') }}"
                        ...
                        <div>
                            ...
                            <input type="url" name="auction_url" id="auction_url" value="{{ old('auction_url') }}"
            ...
            <x-admin.card title="Foto Aset & Lampiran" subtitle="Upload foto aset dan brosur PDF">
                <div class="space-y-4">
                    <div>
                        <label for="images" class="block text-[13px] font-semibold text-zinc-700 dark:text-slate-300 mb-1.5">Foto Aset Agunan</label>
                        ...
                        <p class="mt-1 text-xs text-zinc-500">Dapat memilih lebih dari 1 foto (Format JPG, PNG, WEBP)</p>
                        @error('images')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="status" id="status" class="w-full px-4 py-2.5 border dark:border-slate-700 border-zinc-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-colors text-sm @error('status') border-red-500 @enderror" required>
                            <option value="draft" {{ old('status', 'published') === 'draft' ? 'selected' : '' }}>Draft (Sembunyi)</option>
                            <option value="published" {{ old('status', 'published') === 'published' ? 'selected' : '' }}>Dipublikasi (Aktif)</option>
                            <option value="registration_open" {{ old('status') === 'registration_open' ? 'selected' : '' }}>Pendaftaran Dibuka</option>
