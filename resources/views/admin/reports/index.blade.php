...
<x-admin.card :noPadding="true">
 ...
 class="w-full sm:flex-1 sm:min-w-[200px] rounded-sm border-zinc-300 text-xs">
 <div class="flex flex-wrap gap-3">
 <select name="type" class="flex-1 sm:flex-none rounded-sm border-zinc-300 text-xs">
 ...
 <select name="year" class="flex-1 sm:flex-none rounded-sm border-zinc-300 text-xs">
 ...
 <a href="{{ route('admin.reports.index') }}" class="inline-flex items-center px-4 py-2 text-xs font-medium text-zinc-700 bg-white rounded-sm">
 Reset
 </a>
 @endif
 </div>
 ...
 <div class="bg-white border border-zinc-200 rounded-sm p-4">
 <div class="mb-3">
 <p class="font-semibold text-gray-900 line-clamp-2">{{ $report->title }}</p>
 <p class="text-xs text-zinc-500 mt-1">{{ $report->file_size ? number_format($report->file_size / 1024 / 1024, 2) . ' MB' : '-' }}</p>
 </div>
 ...
 @if($report->file_path)
 <a href="{{ \App\Helpers\StorageHelper::url($report->file_path) }}" target="_blank" class="flex-1 text-center py-2 text-xs font-medium text-sky-600 hover:bg-sky-100 rounded-sm">
 Lihat
 </a>
 @endif
 <a href="{{ route('admin.reports.edit', $report) }}" class="flex-1 text-center py-2 text-xs font-medium text-sky-600 rounded-sm">
 Edit
 </a>
 <button type="button" data-open-modal="deleteReport{{ $report->id }}" class="flex-1 py-2 text-xs font-medium text-red-600 rounded-sm">
 Hapus
 </button>
 </div>
 </div>
 @empty
 <div class="text-center py-8 text-zinc-500">Belum ada laporan.</div>
 @endforelse
 </div>
 ...
 <p class="text-xs text-zinc-500">{{ $report->file_size ? number_format($report->file_size / 1024 / 1024, 2) . ' MB' : '-' }}</p>
 </div>
 </td>
 <td class="px-4 py-3">
 <x-admin.badge variant="info">{{ $typeLabels[$report->type] ?? $report->type }}</x-admin.badge>
 ...
 <td class="px-4 py-3 text-xs text-zinc-500">{{ number_format($report->download_count ?? 0) }}x</td>
 <td class="px-4 py-3">
 <div class="flex items-center gap-1">
 @if($report->file_path)
 <a href="{{ \App\Helpers\StorageHelper::url($report->file_path) }}" target="_blank" class="p-1.5 text-zinc-500 hover:bg-sky-100 rounded-sm">
 ...
 @endif
 <a href="{{ route('admin.reports.edit', $report) }}" class="p-1.5 text-zinc-500 rounded-sm">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
 </svg>
 </a>
 <button type="button" data-open-modal="deleteReport{{ $report->id }}" class="p-1.5 text-zinc-500 rounded-sm hover:text-red-600">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 @empty
 <tr><td colspan="6" class="px-4 py-8 text-center text-zinc-500">Belum ada laporan.</td></tr>
 @endforelse
 </x-admin.table>
 </div>

 @if($reports->hasPages())
 <div class="p-4 border-t border-zinc-200">{{ $reports->links() }}</div>
 @endif
</x-admin.card>
