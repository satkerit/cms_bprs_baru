...
<div class="mb-4 grid grid-cols-1 md:grid-cols-2 gap-6">
 ...
 <p class="text-[11px] text-zinc-500">Total Storage</p>
 <p class="text-4xl font-semibold text-zinc-900">{{ number_format($storageInfo['total'] / 1024 / 1024 / 1024, 2) }} GB</p>
 </div>
 <div class="bg-white rounded-xl p-4 border border-zinc-200">
 <p class="text-[11px] text-zinc-500">Terpakai</p>
 <p class="text-4xl font-semibold text-sky-600">{{ number_format($storageInfo['used'] / 1024 / 1024 / 1024, 2) }} GB</p>
 </div>
 <div class="bg-white rounded-xl p-4 border border-zinc-200">
 <p class="text-[11px] text-zinc-500">Tersedia</p>
...
<x-admin.card :noPadding="true">
 ...
 <div class="bg-zinc-50 rounded-xl p-3 flex items-center gap-3">
 @if($item['type'] === 'folder')
 ...
 @if($item['type'] === 'folder')
 <a href="{{ route('admin.storage.index', ['path' => $item['path']]) }}" class="font-medium text-zinc-900 block">
 {{ $item['name'] }}
 </a>
 @else
 <p class="font-medium text-zinc-900">{{ $item['name'] }}</p>
 <p class="text-[13px] text-zinc-500">{{ number_format($item['size'] / 1024, 1) }} KB</p>
 @endif
 </div>
 <div class="flex items-center gap-1">
 @if($item['type'] === 'file')
 <a href="{{ route('admin.storage.download', ['file' => $item['path']]) }}" class="p-2 text-zinc-500 hover:bg-sky-100 rounded-xl">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
 </svg>
 </a>
 @endif
 <button data-path="{{ $item['path'] }}" data-type="{{ $item['type'] }}" data-name="{{ $item['name'] }}" class="js-delete-btn p-2 text-zinc-500 rounded-xl">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 @empty
 <div class="text-center py-8 text-zinc-500">Folder kosong.</div>
 @endforelse
 </div>
 ...
 <a href="{{ route('admin.storage.index', ['path' => $item['path']]) }}" class="font-medium text-zinc-900">
 {{ $item['name'] }}
 </a>
 @else
 ...
 <div>
 <p class="font-medium text-zinc-900">{{ $item['name'] }}</p>
 <p class="text-[11px] text-zinc-500">{{ $item['extension'] ?? 'file' }}</p>
 </div>
 @endif
 </div>
 </td>
 ...
 <a href="{{ $item['url'] }}" target="_blank" class="p-1.5 text-zinc-500 hover:bg-sky-100 rounded-xl" title="Preview">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 @endif
 <a href="{{ route('admin.storage.download', ['file' => $item['path']]) }}" class="p-1.5 text-zinc-500 rounded-xl" title="Download">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
 </svg>
 </a>
 @endif
 <button data-path="{{ $item['path'] }}" data-type="{{ $item['type'] }}" data-name="{{ $item['name'] }}" class="js-rename-btn p-1.5 text-zinc-500 hover:bg-amber-100 rounded-xl" title="Rename">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
 </svg>
 </button>
 <button data-path="{{ $item['path'] }}" data-type="{{ $item['type'] }}" data-name="{{ $item['name'] }}" class="js-delete-btn p-1.5 text-zinc-500 rounded-xl" title="Hapus">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
 </svg>
 </button>
 ...
 @empty
 <tr><td colspan="4" class="px-4 py-8 text-center text-zinc-500">Folder kosong.</td></tr>
 @endforelse
 </x-admin.table>
 </div>
</x-admin.card>
...
<div id="uploadModal" class="hidden fixed bg-black z-50 flex items-center justify-center p-4">
 ...
 <h3 class="text-3xl font-semibold text-zinc-900 mb-4">Upload File</h3>
 <form action="{{ route('admin.storage.upload') }}" method="POST" enctype="multipart/form-data">
 @csrf
 <input type="hidden" name="path" value="{{ $path }}">
 <div class="mb-4">
 <label class="block text-[11px] font-medium text-zinc-700 mb-2">Pilih File</label>
 <input type="file" name="files[]" multiple required class="w-full text-[13px] text-zinc-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-medium file:bg-emerald-50 file:text-emerald-700 hover:file:bg-sky-100">
 <p class="text-[11px] text-zinc-500 mt-1">Maksimal 50MB per file</p>
 </div>
 <div class="flex justify-end gap-3">
 <button type="button" data-close-modal="uploadModal" class="px-4 py-2 text-[11px] font-medium text-zinc-700 rounded-xl">Batal</button>
...
<div id="folderModal" class="hidden fixed bg-black z-50 flex items-center justify-center p-4">
 <div class="bg-white rounded-xl max-w-md w-full p-6">
 <h3 class="text-3xl font-semibold text-zinc-900 mb-4">Buat Folder Baru</h3>
 <form action="{{ route('admin.storage.create-folder') }}" method="POST">
 @csrf
 <input type="hidden" name="path" value="{{ $path }}">
 <div class="mb-4">
 <label class="block text-[11px] font-medium text-zinc-700 mb-2">Nama Folder</label>
 ...
 <p class="text-[11px] text-zinc-500 mt-1">Hanya huruf, angka, dash, dan underscore</p>
 </div>
 <div class="flex justify-end gap-3">
 <button type="button" data-close-modal="folderModal" class="px-4 py-2 text-[11px] font-medium text-zinc-700 rounded-xl">Batal</button>
...
<div id="deleteModal" class="hidden fixed bg-black z-50 flex items-center justify-center p-4">
 <div class="bg-white rounded-xl max-w-md w-full p-6">
 <h3 class="text-3xl font-semibold text-zinc-900 mb-2">Konfirmasi Hapus</h3>
 <p class="text-zinc-700 mb-4">Yakin ingin menghapus <span id="deleteItemName" class="font-medium"></span>?</p>
 ...
 <div class="flex justify-end gap-3">
 <button type="button" data-close-modal="deleteModal" class="px-4 py-2 text-[11px] font-medium text-zinc-700 rounded-xl">Batal</button>
...
<div id="renameModal" class="hidden fixed bg-black z-50 flex items-center justify-center p-4">
 <div class="bg-white rounded-xl max-w-md w-full p-6">
 <h3 class="text-3xl font-semibold text-zinc-900 mb-4">Ubah Nama</h3>
 ...
 <div class="mb-4">
 <label class="block text-[11px] font-medium text-zinc-700 mb-2">Nama Baru</label>
 <input type="text" name="new_name" id="renameNewName" required class="w-full rounded-xl border-zinc-300 text-[13px]">
 </div>
 <div class="flex justify-end gap-3">
 <button type="button" data-close-modal="renameModal" class="px-4 py-2 text-[11px] font-medium text-zinc-700 rounded-xl">Batal</button>
