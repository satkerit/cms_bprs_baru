...
@if(session('error'))
<div class="mb-6 p-4 bg-red-100 border border-red-200 text-red-600 rounded-xl">
 {{ session('error') }}
</div>
@endif
...
<x-admin.card :noPadding="true">
 ...
 class="w-full border-zinc-300 text-[13px]">
 <div class="flex flex-wrap gap-3">
 <input type="date" name="date_from" value="{{ request('date_from') }}"
 placeholder="Dari Tanggal"
 class="border-zinc-300 text-[13px]">
 ...
 <!-- Bulk Actions Bar -->
 <div id="bulkActionsBar" class="hidden p-4 bg-sky-100 border-b border-sky-200">
 <div class="flex items-center justify-between">
 <div class="flex items-center gap-3">
 <span id="selectedCount" class="text-[11px] font-medium text-blue-900">0 item dipilih</span>
 <div class="flex items-center gap-2">
 <button id="bulkActivateBtn" class="px-3 py-1.5 text-[11px] font-medium text-amber-700 bg-sky-100 rounded-lg">
 Aktifkan
 </button>
 <button id="bulkDeactivateBtn" class="px-3 py-1.5 text-[11px] font-medium text-emerald-700 bg-emerald-100 rounded-lg">
 Nonaktifkan
 </button>
 <button id="bulkDeleteBtn" class="px-3 py-1.5 text-[11px] font-medium text-red-600 bg-red-100 rounded-lg">
 Hapus
 </button>
 </div>
 </div>
 <button id="clearSelectionBtn" class="text-[13px] text-sky-600 hover:text-sky-700">
 Batal Pilih
 </button>
 </div>
 </div>
 ...
 <button type="button" data-open-modal="deleteSchedule{{ $schedule->id }}" class="flex-1 py-2 text-[11px] font-medium text-red-600 border border-red-200 hover:border-red-400 rounded-xl transition-colors">
 Hapus
 </button>
 </div>
 </div>
 ...
 <tr>
 <td class="px-4 py-3">
 <input type="checkbox" class="schedule-checkbox rounded border-zinc-300 text-sky-600"
...
@if($schedules->count())
 ...
 id="deleteSchedule{{ $schedule->id }}"
 title="Hapus Jadwal"
 message="Apakah Anda yakin ingin menghapus jadwal kas keliling di \"{{ $schedule->location }}\" pada {{ $schedule->schedule_date->format('d M Y') }}? Tindakan ini tidak dapat dibatalkan."
 action="{{ route('admin.kas-keliling.destroy', $schedule) }}"
 />
 @endforeach
@endif
