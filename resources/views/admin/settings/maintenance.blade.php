...
<div class="container max-w-5xl">
 ...
 <p class="text-xs text-gray-500">Website akan menampilkan halaman maintenance untuk pengunjung</p>
 </div>
 ...
 <div>
 <label class="block text-xs font-medium text-gray-700 mb-1">Pesan Maintenance</label>
 <textarea name="maintenance_message" rows="3" class="block w-full rounded-lg border-gray-300 text-xs">{{ old('maintenance_message', $settings->maintenance_message) }}</textarea>
 ...
 <div>
 <label class="block text-xs font-medium text-gray-700 mb-1">Daftar IP</label>
 <textarea name="maintenance_allowed_ips" rows="4" class="block w-full rounded-lg border-gray-300 text-xs font-mono" placeholder="192.168.1.1&#10;10.0.0.1">{{ old('maintenance_allowed_ips', $settings->maintenance_allowed_ips) }}</textarea>
 ...
 <div class="space-y-4">
 <p class="text-xs text-gray-700">Pilih halaman yang ingin di-maintenance (kosongkan untuk maintenance seluruh website):</p>
 ...
 <h4 class="font-medium text-gray-700 text-xs">{{ $groupName }}</h4>
 </div>
 <div class="p-3 grid grid-cols-1 md:grid-cols-2 gap-6 md:grid-cols-2 gap-2">
 @foreach($pageKeys as $key)
 @if(isset($availablePages[$key]))
 <label class="flex items-center gap-2 p-2 bg-gray-50 rounded-lg {{ str_starts_with($key, 'about_') || str_starts_with($key, 'products_') || str_starts_with($key, 'reports_') ? 'ml-4' : '' }}">
 ...
 <span class="text-xs text-gray-700">{{ $availablePages[$key]['name'] }}</span>
 </label>
 @endif
 @endforeach
 </div>
