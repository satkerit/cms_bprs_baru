        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <x-admin.card title="Informasi Anggota" subtitle="Data utama anggota dewan">
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="type" class="block text-sm font-semibold text-zinc-700 mb-1.5 ml-0.5">
                                Tipe Anggota <span class="text-red-500">*</span>
                            </label>
                            <select name="type" id="type" required class="block w-full rounded-xl border-0 py-2.5 px-4 text-zinc-900 bg-zinc-50 shadow-sm ring-1 ring-inset ring-zinc-200 focus:bg-white focus:ring-2 focus:ring-inset focus:ring-sky-500 sm:text-sm sm:leading-6 hover:ring-zinc-300 transition-all">
                    ...
                    <div>
                        <label for="biography" class="block text-sm font-semibold text-zinc-700 mb-1.5 ml-0.5">Biografi</label>
                        <textarea name="biography" id="biography" rows="5" placeholder="Ceritakan latar belakang dan pengalaman anggota ini..." class="block w-full rounded-xl border-0 py-2.5 px-4 text-zinc-900 bg-zinc-50 shadow-sm ring-1 ring-inset ring-zinc-200 placeholder:text-zinc-400 focus:bg-white focus:ring-2 focus:ring-inset focus:ring-sky-500 sm:text-sm sm:leading-6 hover:ring-zinc-300 transition-all @error('biography') ring-red-300 focus:ring-red-500 bg-red-50/50 @enderror">{{ old('biography', $boardMember->biography ?? '') }}</textarea>
                        @error('biography')
                            <p class="mt-1.5 text-xs text-red-600 font-medium ml-0.5">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="flex gap-2 group">
                            <div class="relative flex-1">
                                ...
                                <input type="text"
                                       ...
                                       class="w-full pl-10 rounded-xl border-0 py-2.5 px-4 text-zinc-900 bg-zinc-50 shadow-sm ring-1 ring-inset ring-zinc-200 placeholder:text-zinc-400 focus:bg-white focus:ring-2 focus:ring-inset focus:ring-sky-500 sm:text-sm sm:leading-6 hover:ring-zinc-300 transition-all"
                                       placeholder="Contoh: S1 Ekonomi - Universitas Indonesia">
                            </div>
                            <button type="button"
                                    @click="removeItem(index)"
                                    x-show="items.length > 1"
                                    class="p-2.5 text-zinc-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all opacity-0 group-hover:opacity-100">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
