<div>
    ...
    <div class="text-center py-8" x-data x-init="$el.classList.add('animate-scale-in')">
        ...
        <p class="text-sm text-secondary mb-8 max-w-md mx-auto">Terima kasih. Tim kami akan menindaklanjuti laporan Anda sesuai prosedur yang berlaku.</p>
        ...
        <div class="inline-block border border-emerald-200 dark:border-emerald-700/50 bg-emerald-50/60 dark:bg-emerald-900/20 px-8 py-6 mb-8">
            ...
            <p class="mt-2 text-xs text-emerald-700/80 dark:text-emerald-500">Simpan nomor ini untuk memantau status laporan Anda.</p>
        </div>
    ...
    <form wire:submit="submit" class="space-y-8" novalidate>
        ...
        <div class="flex items-start gap-4 p-4 sm:p-5 border {{ $errors->has('is_anonymous') ? 'border-red-300 dark:border-red-700' : 'border-amber-200 dark:border-amber-700/40' }} bg-amber-50/70 dark:bg-amber-900/10">
            ...
            <label for="is_anonymous" class="cursor-pointer">
                ...
                <span class="block text-xs text-amber-800/80 dark:text-amber-500 mt-1 leading-relaxed">Identitas Anda dirahasiakan sepenuhnya. Catatan: kami tidak dapat menghubungi Anda untuk klarifikasi lanjutan.</span>
            </label>
        </div>
        ...
        <section class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-foreground dark:text-slate-200 mb-1.5">Nomor Telepon <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-medium text-foreground dark:text-slate-200 mb-1.5">No. Rekening / Nasabah <span class="text-secondary font-normal text-xs">(opsional)</span></label>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" wire:model.live="identity_number" placeholder="16 digit nomor KTP/SIM"
                           class="{{ $inputBase }} {{ $inputOk }}">
                    <p class="mt-1.5 text-xs text-secondary">Diperlukan jika pengaduan berkaitan dengan transaksi atas nama Anda.</p>
                </div>
            </div>
        </section>
        @endif
        ...
        <section class="space-y-5">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-sm font-bold text-foreground dark:text-white tracking-wide">Detail Pengaduan</h3>
                <span class="flex-1 h-px bg-border dark:bg-slate-700"></span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-medium text-foreground dark:text-slate-200 mb-1.5">Kategori <span class="text-red-500">*</span></label>
                    <div class="relative">
                ...
                <div>
                    <label class="block text-sm font-medium text-foreground dark:text-slate-200 mb-1.5">Subjek <span class="text-red-500">*</span></label>
            ...
            <div>
                ...
                <textarea wire:model.live="description" rows="6" maxlength="3000"
                ...
                <div class="mt-2 flex items-center justify-between gap-4">
                    <div class="flex-1 h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                        <div class="h-full rounded-full transition-all duration-300 {{ strlen($description) > 2800 ? 'bg-amber-500' : 'bg-emerald-500' }}"
                             style="width: {{ min(100, strlen($description) / 3000 * 100) }}%"></div>
                    </div>
                    @error('description')
                        <p class="text-xs text-red-600 dark:text-red-400 shrink-0">{{ $message }}</p>
                    @else
                        <p class="text-xs text-secondary shrink-0">Min. 20 karakter</p>
                    @enderror
                    <p class="font-mono text-xs {{ strlen($description) > 2800 ? 'text-amber-600 dark:text-amber-400' : 'text-secondary' }} shrink-0">{{ strlen($description) }}/3000</p>
                </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-sm font-medium text-foreground dark:text-slate-200 mb-1.5">Kantor Cabang <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <select wire:model.live="branch_office"
                                class="w-full appearance-none pr-10 {{ $inputBase }} {{ $errors->has('branch_office') ? $inputErr : $inputOk }}">
                    ...
                    @error('branch_office')<p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-foreground dark:text-slate-200 mb-1.5">Tanggal Kejadian <span class="text-secondary font-normal text-xs">(opsional)</span></label>
        ...
        <section class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-sm font-bold text-foreground dark:text-white tracking-wide">Lampiran</h3>
                <span class="flex-1 h-px bg-border dark:bg-slate-700"></span>
            ...
            <label for="attachments"
                ...
                <span class="min-w-0">
                    <span class="block text-sm font-medium text-foreground dark:text-slate-200">Unggah bukti pendukung</span>
