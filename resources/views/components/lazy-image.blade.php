...
<div class="relative overflow-hidden {{ $containerClass }}" style="--aspect-ratio: {{ $aspectRatio }};">
    @if($hasImage)
        <img
            ...
            class="absolute inset-0 w-full h-full transition-all duration-300 {{ $class }} lazy-image"
            style="object-fit: {{ $objectFit }}; background-image: url('{{ $placeholder }}'); background-size: cover;";
            data-fallback-bg="{{ $fallbackBg }}"
        >
    @else
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('img[data-fallback-bg]').forEach(function(img) {
        ...
        img.addEventListener('error', function() {
            var bg = this.dataset.fallbackBg || 'from-gray-100 to-gray-200';
            var div = document.createElement('div');
            div.className = 'absolute inset-0 w-full h-full bg-gradient-to-br ' + bg + ' flex items-center justify-center';
