...
<div class="hero-slider-container"
    ...
    @if(count($slides) > 0)
        ...
        @if($showNavigation && count($slides) > 1)
            <button class="hero-nav prev" data-slider="prev">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/>
                </svg>
            </button>
            <button class="hero-nav next" data-slider="next">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div class="hero-dots">
                @foreach($slides as $index => $slide)
                    <button class="hero-dot {{ $index === 0 ? 'active' : '' }}"
                            data-slider="goto" data-slide-index="{{ $index }}"></button>
                @endforeach
            </div>
        @endif
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Bind data-slider navigation
    document.querySelectorAll('[data-slider="prev"]').forEach(function(btn) {
        btn.addEventListener('click', function() { if (window.heroSlider) window.heroSlider.prev(); });
    });
    document.querySelectorAll('[data-slider="next"]').forEach(function(btn) {
        btn.addEventListener('click', function() { if (window.heroSlider) window.heroSlider.next(); });
    });
    document.querySelectorAll('[data-slider="goto"]').forEach(function(btn) {
        btn.addEventListener('click', function() { if (window.heroSlider) window.heroSlider.goTo(parseInt(this.dataset.slideIndex)); });
    });
});
