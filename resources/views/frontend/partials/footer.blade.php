...
<footer class="bg-primary text-white relative overflow-hidden">
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div class="absolute -top-20 -left-20 w-72 h-72 bg-emerald-600/10 rounded-full blur-3xl animate-float" style="animation-delay: 0s; animation-duration: 8s;"></div>
        <div class="absolute top-1/3 -right-16 w-80 h-80 bg-emerald-600/10 rounded-full blur-3xl animate-float" style="animation-delay: 2s; animation-duration: 10s;"></div>
        <div class="absolute -bottom-32 left-1/4 w-96 h-96 bg-emerald-600/8 rounded-full blur-3xl animate-float" style="animation-delay: 4s; animation-duration: 12s;"></div>
    </div>

    {{-- ─── Elegant Top Border ─── --}}
    <div class="relative h-1.5 w-full bg-gradient-to-r from-emerald-600 via-emerald-500 to-emerald-600 opacity-90"></div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 lg:gap-10">
                ...
                <div class="fade-in-section" x-intersect="$el.classList.add('is-visible')">
                    <div class="mb-6">
                        <a href="{{ route('home') }}" class="inline-flex items-center gap-3 group">
                            ...
                            <img src="{{ \App\Helpers\StorageHelper::url($company->logo) }}"
                                 alt="{{ $company->name }}"
                                 class="h-12 w-auto max-w-[180px] object-contain brightness-0 invert transition-all duration-300 group-hover:brightness-75 group-hover:scale-105">
                            @else
                            <div class="w-11 h-11 bg-gradient-to-br from-emerald-600 to-emerald-700 rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/20 transition-transform duration-300 group-hover:scale-110">
                    ...
                    <div class="space-y-2 mb-6">
                        @if($company->ojk_tagline)
                            <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 rounded-lg border border-emerald-100">
                                <svg class="w-3.5 h-3.5 text-emerald-600 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
                                </svg>
                                <span class="text-sm text-emerald-600 font-medium">{{ $company->ojk_tagline }}</span>
                            </div>
                        @endif
                    ...
                    <div class="flex flex-wrap gap-2 mb-6">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-100 text-emerald-600 rounded-full text-xs font-semibold border border-emerald-200 transition-all duration-300 hover:bg-emerald-100 hover:border-emerald-600/50">
                            ...
                            @foreach($socialLinks as $name => $link)
                                ...
                                <a href="{{ $link['url'] }}" target="_blank"
                                   ...
                                   aria-label="{{ ucfirst($name) }}">
                                    {{-- Glow on hover --}}
                                    <div class="absolute inset-0 rounded-xl bg-emerald-600/0 group-hover:bg-emerald-100 transition-all duration-300"></div>
                                    <svg class="relative w-4 h-4 text-white/70 group-hover:text-emerald-600 transition-colors duration-300" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="{{ $link['path'] }}"/>
                                    </svg>
                                </a>
                                @endif
                    ...
                    <ul class="space-y-2.5">
                        ...
                        <li>
                            <a href="{{ route($link['route']) }}"
                               class="group flex items-center gap-2.5 text-white/80 hover:text-emerald-600 transition-all duration-200 text-sm">
                                <span class="w-1.5 h-1.5 rounded-full bg-white/50 group-hover:bg-emerald-600 transition-all duration-200"></span>
                ...
                <div class="fade-in-section" x-intersect="$el.classList.add('is-visible')" style="transition-delay: 200ms;">
                    ...
                    <div class="space-y-4">
                        @if($company->address)
                        <div class="group flex gap-3.5 p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/[0.07] hover:border-emerald-100 transition-all duration-300">
                            <div class="w-10 h-10 rounded-lg bg-emerald-50 border border-emerald-100 flex items-center justify-center shrink-0 group-hover:bg-emerald-100 transition-all duration-300">
                        ...
                        <div class="group flex gap-3.5 p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/[0.07] hover:border-emerald-100 transition-all duration-300">
                            ...
                            <div class="min-w-0">
                                ...
                                <a href="tel:{{ $company->phone }}" class="text-sm text-white/80 font-semibold hover:text-emerald-600 transition-colors">{{ $company->phone }}</a>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="group flex gap-3.5 p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/[0.07] hover:border-emerald-100 transition-all duration-300">
                            ...
                            <div class="min-w-0">
                                <p class="text-[11px] text-white/60 font-medium uppercase tracking-wider mb-0.5">WhatsApp</p>
                                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $company->whatsapp) }}" target="_blank" class="text-sm text-white/80 font-semibold hover:text-emerald-600 transition-colors">{{ $company->whatsapp }}</a>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="group flex gap-3.5 p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/[0.07] hover:border-emerald-100 transition-all duration-300">
                            ...
                            <div class="min-w-0">
                                ...
                                <a href="mailto:{{ $company->email }}" class="text-sm text-white/80 font-semibold hover:text-emerald-600 transition-colors">{{ $company->email }}</a>
                            </div>
                        </div>
                        @endif
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-4 text-xs">
                    <a href="{{ route('privacy-policy') }}" class="text-white/60 hover:text-emerald-600 transition-colors duration-200 relative group">
                        Kebijakan Privasi
                        <span class="absolute -bottom-0.5 left-0 w-0 h-px bg-emerald-600 group-hover:w-full transition-all duration-300"></span>
                    </a>
                    <span class="text-white/40">•</span>
                    <a href="#" class="text-white/60 hover:text-emerald-600 transition-colors duration-200 relative group">
                        Syarat & Ketentuan
                        <span class="absolute -bottom-0.5 left-0 w-0 h-px bg-emerald-600 group-hover:w-full transition-all duration-300"></span>
                    </a>
                    <span class="text-white/40">•</span>
