        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($reports->count() > 0)
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8" x-data="{ year: '{{ request('year', '') }}' }">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select id="year-filter"
                                x-model="year"
                                @change="year ? window.location = '{{ route('reports.' . str_replace('_', '-', $type) . '') }}?year=' + year : window.location = '{{ route('reports.' . str_replace('_', '-', $type) . '') }}'"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    ...
                    <div class="group bg-card rounded-xl border border-border overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col">
                        ...
                        <div class="relative h-32 sm:h-36 bg-gradient-to-br from-emerald-50 to-emerald-100 flex items-center justify-center overflow-hidden">
                            ...
                            <div class="relative group-hover:scale-110 transition-transform duration-300">
                                <svg class="w-14 h-14 sm:w-16 sm:h-16 text-emerald-600/80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                        ...
                        <div class="p-4 sm:p-5 flex flex-col flex-1">
                            <h3 class="font-bold text-foreground group-hover:text-emerald-600 transition-colors leading-snug mb-1.5">
                                {{ $report->title }}
                            </h3>
                            ...
                            <div class="flex items-center gap-2 pt-3 border-t border-border">
                                <a href="{{ route('reports.preview', $report) }}" target="_blank" rel="noopener"
                                   class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-600 text-white text-xs font-semibold rounded-lg hover:bg-emerald-700 transition-colors btn-press">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    Preview
                                </a>
                                <a href="{{ route('reports.download', $report) }}"
                                   class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-white dark:bg-slate-800 dark:text-emerald-400 text-emerald-700 text-xs font-semibold rounded-lg border border-emerald-200 dark:border-emerald-800/50 hover:bg-emerald-50 dark:hover:bg-slate-700 transition-colors btn-press">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                    Download
                                </a>
                            </div>
            ...
            @else
                <div class="text-center py-16 sm:py-20 bg-card rounded-xl border border-border">
                    ...
                    <a href="{{ route('reports.index') }}" class="inline-flex items-center gap-1.5 mt-6 px-4 py-2.5 bg-emerald-600 text-white text-sm font-semibold rounded-lg hover:bg-emerald-700 transition-colors">
