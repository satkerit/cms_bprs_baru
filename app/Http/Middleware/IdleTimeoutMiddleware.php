...
class IdleTimeoutMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // 30-second grace period on top of idleTimeout to handle clock skew & cache desync
        $gracePeriod = 30;
        if ($timeIdle > $idleTimeoutSeconds + $gracePeriod) {
            // Log idle logout
            AuditTrail::log('idle_logout', 'User logged out due to inactivity: ' . $user->name);

            // Clean up security fingerprint set by SecureSessionMiddleware
            Session::forget('security_fingerprint');
            Session::forget('cached_role');

            // Clear activity cache key
        ...
        // Tambahkan informasi idle timeout ke response header untuk JavaScript.
        // Hanya kirim nilai relatif (detik), bukan timestamp absolut untuk mencegah information leakage.
        if (Auth::check()) {
            $secondsUntilTimeout = max(0, $idleTimeoutSeconds - ($currentTime - $lastActivity));
            $response->headers->set('X-Idle-Timeout', $idleTimeout * 60);
            $response->headers->set('X-Idle-Remaining', $secondsUntilTimeout);
