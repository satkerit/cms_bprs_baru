...
class StorageServeController extends Controller
{
    ...
    {
        // Validasi path tidak mengandung directory traversal
        ...
        // Validasi ekstensi file yang diizinkan (whitelist)
        $allowedExtensions = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'pdf', 'doc', 'docx', 'xls', 'xlsx', 'mp4', 'webm'];
        $extension = strtolower(pathinfo($path, PATHINFO_EXTENSION));
        
        if (!in_array($extension, $allowedExtensions)) {
            Log::warning('StorageServe: Blocked file extension', [
                'path' => $path,
                'extension' => $extension,
            ]);
