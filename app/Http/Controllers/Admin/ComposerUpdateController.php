    ...
    public function runUpdate(Request $request)
    {
        ...
        if (!auth()->user()->isSuperAdmin()) {
            return response()->json([
                'success' => false,
                'message' => 'Hanya Super Admin yang dapat menjalankan composer update.',
                'output' => null,
            ], 403);
        }
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error('ComposerUpdateController runUpdate error', [
                'message' => $e->getMessage(),
                'trace'   => $e->getTraceAsString(),
            ]);
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan saat menjalankan composer update. Silakan periksa log server.',
                'output'  => null,
            ], 500);
        }
    }
