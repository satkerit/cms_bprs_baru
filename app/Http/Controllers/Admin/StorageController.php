...
class StorageController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view(
            "admin.storage.index",
            compact("items", "path", "breadcrumbs", "storageInfo"),
        );
    }
    ...
    public function upload(Request $request)
    {
        ...
        if ($request->hasFile("files")) {
            foreach ($request->file("files") as $file) {
                ...
                AuditTrail::log(
                    'upload',
                    "Upload file: {$filename} ke {$path}",
                    null,
                    null,
                    ['filename' => $filename, 'path' => $path, 'size' => $file->getSize(), 'mime' => $file->getMimeType()]
                );
            }
        }
    ...
    public function delete(Request $request)
    {
        ...
        return redirect()
            ->route("admin.storage.index", ["path" => $parentPath])
    ...
    public function rename(Request $request)
    {
        ...
        // Pastikan ekstensi file tidak berubah saat rename untuk mencegah eksekusi file berbahaya.
        if ($request->type === "file") {
            $oldExt = strtolower(pathinfo($oldPath, PATHINFO_EXTENSION));
            $newExt = strtolower(
                pathinfo($request->new_name, PATHINFO_EXTENSION),
            );
            if ($oldExt !== $newExt) {
                return back()->with(
                    "error",
                    "Ekstensi file tidak boleh diubah saat rename.",
                );
            }
        }
        ...
        try {
            ...
            return response()->json(
                [
                    "error" => "Server error",
                    "message" =>
                    "Terjadi kesalahan saat memuat direktori. Silakan coba lagi.",
                    "path" => $request->get("path", ""),
                    "items" => [],
                ],
                500,
