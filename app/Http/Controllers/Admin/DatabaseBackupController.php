...
class DatabaseBackupController extends Controller
{
    ...
    public function index()
    {
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::error('DatabaseBackupController index error', [
    ...
    protected function getBackupMetadata($filePath): array
    {
        $isCompressed = pathinfo($filePath, PATHINFO_EXTENSION) === 'gz';

        // Baca hanya 2KB pertama file untuk ekstrak metadata header,
        // bukan seluruh konten file (bisa ratusan MB) untuk mencegah OOM.
        $handle = @fopen($filePath, 'rb');
        if (!$handle) {
            return [];
        }

        $rawChunk = fread($handle, 2048);
        fclose($handle);

        $chunk = $isCompressed ? @zlib_decode($rawChunk) : $rawChunk;
        if ($chunk === false || empty($chunk)) {
            return [];
        }

        if (preg_match('/-- Backup Metadata:(.*?)-- End Metadata/s', $chunk, $matches)) {
            $metadata = [];
    ...
    protected function generatePdoBackup($backupType, $description = '')
    {
        ...
        foreach ($tables as $table) {
            ...
            // Escape backticks in table name for safe SQL interpolation
            $safeTable = str_replace('`', '``', $tableName);

            if ($backupType !== 'data_only') {
                $sql .= "-- Structure for `{$safeTable}`\n\n";

                $sql .= ($tableType === 'VIEW')
                    ? "DROP VIEW IF EXISTS `{$safeTable}`;\n"
                    : "DROP TABLE IF EXISTS `{$safeTable}`;\n";

                $createTable = DB::select("SHOW CREATE TABLE `{$safeTable}`");
            ...
            if ($backupType !== 'structure_only' && $tableType === 'BASE TABLE') {
                $rowCount = DB::table($safeTable)->count();
                if ($rowCount > 0) {
                    $sql .= "-- Dumping data for table `{$safeTable}` ({$rowCount} rows)\n\n";
                    $columns = DB::getSchemaBuilder()->getColumnListing($safeTable);
                    $columnList = '`' . implode('`, `', $columns) . '`';
                    DB::table($safeTable)->chunk(1000, function ($rows) use (&$sql, $safeTable, $columns, $columnList) {
                        foreach ($rows as $row) {
                            $rowArray = (array) $row;
