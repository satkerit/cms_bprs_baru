...
use App\Http\Requests\Admin\KasKeliling\StoreKasKelilingRequest;
use App\Http\Requests\Admin\KasKeliling\UpdateKasKelilingRequest;
...
class KasKelilingController extends Controller
{
    ...
    public function store(StoreKasKelilingRequest $request)
    {
        $this->authorizeCreate('kas_keliling.manage');
        try {
    ...
    public function update(UpdateKasKelilingRequest $request, $id)
    {
        $this->authorizeEdit('kas_keliling.manage');
        try {
            $kasKeliling = KasKelilingSchedule::findOrFail($id);
