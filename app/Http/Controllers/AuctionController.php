...
class AuctionController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Sort — prioritaskan item non-sold (aktif) di atas sold (terjual)
        $query->orderByRaw("CASE WHEN status = 'sold' THEN 1 ELSE 0 END");
        ...
        $assetTypes = \Illuminate\Support\Facades\Cache::remember('auctions_asset_types', 86400, function () {
            return Auction::published()
                ...
                ->mapWithKeys(function ($type) {
                    return [$type => AssetType::tryFrom($type)?->label() ?? $type];
                });
        });
    ...
    {
        if (!AssetType::tryFrom($assetType)) {
            abort(404);
        }
        ...
        $assetTypeName = AssetType::from($assetType)->label();
